...
const STOREY = 8.6;           // floor to floor - headroom to jump under
const VOID = 13;              // half-width of the open shaft
const RING = 27;              // half-width of the building (a 14-wide gallery)
...
function tiles(cx, cz, w, d, top, a, b) {
  const step = 5.4, n = Math.floor(w / step), m = Math.floor(d / step);
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < m; j++) {
      if ((i + j) % 2) continue;
      box(step - 0.5, 0.06, step - 0.5, cx - w / 2 + step / 2 + i * step, top + 0.03,
          cz - d / 2 + step / 2 + j * step, a, { solid: false, shadow: false });
    }
  }
}
...
const STAIR_R = VOID - 2.6;          // the flights hang inside the shaft
const STAIR_STEPS = 17, STAIR_RUN = 14.0, STAIR_W = 5.2;
...
function buildAscent(n) {
  ...
  const pts = [];
  const reach = kind === 'lift' ? 4.6 : STAIR_RUN / 2 + 1.9;
  ...
  if (kind === 'stairs') {
    ...
    for (let i = 0; i < cnt; i++) {
      ...
      const wob = i % 2 ? 1.8 : -1.8;
      const p = sidePoint(side, along, STAIR_R + wob);
      const y = y0 + (y1 - y0) * t;
      platform(p.x, y, p.z, 5.6, 5.6, i % 2 ? b.trim : band(n + 1).trim);
    ...
    const car = platform(mid.x, (y0 + y1) / 2, mid.z, 4.0, 4.0, b.trim, { tag: 'mover' });
    ...
    for (const o of [-1, 1]) {
      const rail = sidePoint(side, o * 2.3, STAIR_R);
    ...
    const steps = 34, rise = (y1 - y0) / steps, depth = STAIR_RUN / steps;
    const from = -STAIR_RUN / 2 * s.sign;
...
function parapet(n, side, open) {
  ...
  const half = VOID;
  const y = floorY(n);
  const segs = open ? [[-half, -10.4], [10.4, half]] : [[-half, half]];  // wide enough for the landings
  const bd = band(n);
  for (const [a, b] of segs) {
...
function buildFloor(n) {
  ...
  for (let side = 0; side < 4; side++) {
    ...
    if (s.ax === 'x') {
      ...
      for (const wx of [-17, 0, 17]) window_(wx, y + 3.8, s.fixed * RING, 2.4, 4.4, 0);
    } else {
      ...
      for (const wz of [-17, 0, 17]) window_(s.fixed * RING, y + 3.8, wz, 2.4, 4.4, Math.PI / 2);
    }
  }
  ...
  // a torch on two of the inner corners
  const tc = n % 2 ? 1 : -1;
  torch(tc * (VOID + 1.4), y + 3.4, VOID + 1.4);
  torch(-tc * (VOID + 1.4), y + 3.4, -(VOID + 1.4));

  // banners hung from the gallery lip into the shaft
  if (n > 0 && n % 2 === 1) {
    for (const side of [0, 2]) {
      const p = sidePoint(side, side === 0 ? -8.5 : 8.5, VOID - 0.4);
      const bn = box(2.4, 4.4, 0.15, p.x, y - 2.5, p.z, b.trim, { solid: false });
...
for (let n = 0; n < FLOORS; n++) {
  ...
  if (n % 4 === 0) { crate(spot.x, y, spot.z, 1.5); crate(spot.x + 1.7, y, spot.z, 1.2); }
  else if (n % 4 === 2) { barrel(spot.x, y, spot.z); barrel(spot.x + 1.3, y, spot.z + 0.6); }
...
function tick() {
  ...
  const wantDist = hits.length ? Math.max(1.1, hits[0].distance - 0.45) : dist;
  // ease outwards, snap inwards: pillars flicking past must not yank the view
  camDistNow = wantDist < camDistNow
    ? wantDist
    : camDistNow + (wantDist - camDistNow) * (1 - Math.pow(0.02, dt));
  camPos.copy(camTarget).addScaledVector(dir, camDistNow);
  // a respawn or a long fall should cut, not fly the camera through the building
  if (camera.position.distanceToSquared(camPos) > 400) camera.position.copy(camPos);
  else camera.position.lerp(camPos, 1 - Math.pow(0.0005, dt));
